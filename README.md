...
3. **Turn on Pages (one time, required).** Repo → **Settings → Pages → Build and
   deployment → Source = GitHub Actions**. GitHub won't let the workflow create
   the Pages site itself, so this switch must be flipped once by hand.
