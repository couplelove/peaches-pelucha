...
export function PlansTab({ client, me, players, flash }) {
  ...
  useEffect(() => {
    ...
    if (p) { window.__ppPlanPrefill = null; setCompose({ emoji: p.emoji || "💗", title: p.title || "", date: todayISO(), time: "", kind: "invite", notes: "" }); }
  }, []);
  ...
  const save = async () => {
    ...
    const row = { title: c.title.trim(), emoji: (c.emoji || "💗").trim() || "💗", starts_on: c.date,
      starts_at: c.time || null, notes: c.notes.trim() || null, kind: c.kind, created_by: me.id };
    ...
    if (partner) {
      ...
      notifyTurn(client, partner.id,
        c.kind === "invite" ? `💌 Invite from ${me.name}` : `📌 FYI from ${me.name}`,
        `${row.emoji} ${row.title} — ${when}`);
    }
  ...
  return html`<div>
    <div class="card">
      <div class="row between">
        ...
        <button class="btn sm" onClick=${() => setCompose({ emoji: "💗", title: "", date: todayISO(), time: "", kind: "invite", notes: "" })}>＋ Plan</button>
      </div>
    ...
    ${compose && html`<div class="modal-bg" onClick=${(e) => { if (e.target.classList.contains("modal-bg")) setCompose(null); }}>
      <div class="modal">
        ...
        <h3>${compose.kind === "invite" ? "Invite 💌" : "FYI 📌"}</h3>
        ...
        <button class="btn block" disabled=${!compose.title.trim() || !compose.date} onClick=${save}>
          ${compose.kind === "invite" ? `Send invite to ${partner ? partner.emoji + " " + partner.name : "…"}` : "Add to calendar"}
        </button>
      </div>
    </div>`}
  </div>`;
